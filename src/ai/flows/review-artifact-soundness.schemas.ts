...
export const ReviewArtifactInputSchema = z.object({
    ...
    paperContextBefore: z
        .string()
        .optional()
        .describe('Raw LaTeX/prose context from the start of the paper up to (but not including) this artifact.'),
...
const ReviewCategorySchema = z.object({
    ...
    feedback: z.string().describe('Direct answer for this category (no leading labels like "OK:" or verdict tokens).'),
});
...
export const ReviewArtifactOutputSchema = z.object({
    ...
    suggestedImprovement: z.string().optional().describe('Concrete, minimal suggestions to improve correctness/clarity.'),
});
