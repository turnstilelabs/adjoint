    ...
    try {
        ...
        if (!problem || !Array.isArray(proofSteps) || !assistantText) {
            return NextResponse.json(
                { error: 'Invalid payload. Expected { problem: string, proofSteps: Sublemma[], assistantText: string }' },
                { status: 400 }
            );
        }

        // Reuse the same reviseProof flow, but drive it from the assistant's final message text
        const result = await reviseProof({ problem, proofSteps, request: assistantText });
        ...
        return NextResponse.json({ error: 'Failed to reconcile impact from assistant text.' }, { status: 500 });
    }
}
