...
import { getLlmApiKey, getLlmId, getLlmModel, getLlmProvider } from '@/ai/genkit';
...
import { AI_SETTINGS_COOKIE_NAME } from '@/ai/ai-settings-cookie';
import { getLlmContextFromCookieValue, runWithLlmContext } from '@/ai/llm-context';
...
async function withLlmContext<T>(fn: () => Promise<T>): Promise<T> {
  const cookieStore = await cookies();
  const ctx = getLlmContextFromCookieValue(cookieStore.get(AI_SETTINGS_COOKIE_NAME)?.value);
  return runWithLlmContext(ctx, async () => {
    try {
      const provider = getLlmProvider();
      const model = getLlmId();
      const hasKey = Boolean(getLlmApiKey());
      console.info(`[AI][req] source=server-action provider=${provider} model=${model} hasKey=${hasKey}`);
    } catch { }
    return fn();
  });
}
...
export async function validateStatementAction(statement: string) {
  return withLlmContext(async () => {
    ...
    try {
      ...
      return { success: true, ...result, model: getLlmModel() };
    } catch (error) {
...
export async function validateProofAction(problem: string, proofSteps: Sublemma[]) {
  return withLlmContext(async () => {
    ...
    try {
      ...
      return { success: true as const, ...result, model: getLlmModel() };
    } catch (error) {
...
export async function reviewArtifactSoundnessAction(
  ...
  return withLlmContext(async () => {
    ...
    try {
      ...
      return { success: true as const, ...out, model: getLlmModel() };
    } catch (error) {
